function [Acorr] = corrbeg3D(A, offset)
...
if ~isempty (A)
    ...
    Acorr{1}(:,4:6) = Acorr{1}(:,4:6) - offset(1,1:3) .* (Acorr{1}(:,2) - first_frame);
    ...
    if s > 3
        for i = 2:s-2
            ...
            Acorr{i}(:,4:6) = Acorr{i}(:,4:6) - 1.5 .* totoffset(1,1:3) - sumoffset(1:3) - offset(i,1:3) .* (Acorr{i}(:,2) - first_frame);
    ...
    if s > 2
        ...
        Acorr{s-1}(:,4:6) = Acorr{s-1}(:,4:6) - 1.5 .* totoffset(1,1:3) - sumoffset(1:3) - offset(s-1,1:3) .* (Acorr{s-1}(:,2) - first_frame);
    end
